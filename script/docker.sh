...
function build_docker_image {
    ...
    VERSION="$(cat VERSION)"
    docker build . -t builders/vehicle-charging:latest -t builders/vehicle-charging:"$VERSION" --build-arg JAR_FILE="./target/vehicleCharging*.jar"
}
...
function show_application_logs_if_requested {
    if [[ "$1" == "logs" ]]; then
        ...
        docker-compose logs -f vehicle-charging
    fi
}
